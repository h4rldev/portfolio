...
// edited to ts by h4rl

export const oneko = (): void => {
  const isReducedMotion =
    window.matchMedia(`(prefers-reduced-motion: reduce)`).matches === true;

  if (isReducedMotion) return;

  const nekoEl: HTMLElement = document.createElement("div");
  let persistPosition: boolean = true;
  ...
  const init = (): void => {
    ...
    if (persistPosition) {
      const _storedNeko: string | null = window.localStorage.getItem("oneko");
      const storedNeko = _storedNeko ? JSON.parse(_storedNeko) : null;
      if (storedNeko !== null) {
  ...
  const setSprite = (name: SpriteKey, frame: number): void => {
